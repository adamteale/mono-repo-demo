...
import { AtButtonIconPosition, AtButtonProps, AtButtonVariants } from './at-button.types'
...
describe('atoms/at-button', () => {
  it('Renders base button', () => {
    ...
    const { getByText } = render(<AtButton variant={variant}>{children}</AtButton>)
  ...
  it('Should display icon - (left-icon)', () => {
    ...
    const { container } = render(
      <AtButton dataTestId={icon} icon={icon} iconPosition={iconPosition} variant={variant}>
        {children}
      </AtButton>,
    )
