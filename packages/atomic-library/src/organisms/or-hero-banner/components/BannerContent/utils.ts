...
export const getContainerClassnames = (variant: OrHeroBannerVariant, align: OrHeroBannerContentAlignment) => {
  const baseClasses = 'flex relative md:justify-start'

  if (variant !== OrHeroBannerVariant.IMAGE_BANNER) {
    const alignmentClass = align === OrHeroBannerContentAlignment.LEFT ? 'xl:ml-auto' : 'xl:mr-auto'
...
export const getContentClassnames = (
  ...
  const buttonSpacing = Array.isArray(buttons) && buttons.length > 0 ? 'gap-8 md:gap-12' : ''
  const mobileAlignClasses = mobileAlign === OrHeroBannerContentAlignmentMobile.TOP ? 'mt-[4.375rem] md:my-auto' : ''

  if (variant !== OrHeroBannerVariant.IMAGE_BANNER) return `${baseClasses} ${buttonSpacing} w-full md:text-left`
  ...
  if (align === OrHeroBannerContentAlignment.RIGHT)
    return `${baseClasses} ${mobileAlignClasses} text-center gap-6 max-md:items-center md:text-left w-full md:w-[20rem] lg:w-[27.875rem] xl:w-[32rem] 2xl:w-[39.375rem] 3xl:w-[52rem] ${
      variant === OrHeroBannerVariant.IMAGE_BANNER && showTextBackground ? backgroundClasses : ''
    }`
