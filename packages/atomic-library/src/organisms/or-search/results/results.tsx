...
import { AtLink } from '@components-library/common'
...
export const Results = ({
  ...
  const hasResults = useMemo(() => totalAmountOfResults !== 0, [totalAmountOfResults])
  const hasSuggestions = useMemo(() => totalAmountOfSuggestions !== 0, [totalAmountOfSuggestions])
  ...
  const mapItems = useCallback(
    (items: OrSearchItem[] | undefined, displayVariant: SearchItemDisplayVariants) => {
      if (!items) return []

      return items
        ?.map((item, index) => {
          const { link, name, slug, price, image, discountPercentage, description, originalPrice } = item

          if (!name || !slug || !link) return null

          if (displayVariant === SearchItemDisplayVariants.TEXT) {
            return <TextSearchItem key={`item-${index}-${slug}`} link={link} name={name} onClick={onItemClick} />
          }
          ...
          return <CardSearchItem key={`item-${index}-${slug}`} onClick={onItemClick} {...itemProps} />
        })
        .filter(Boolean)
  ...
  return (
    ...
    >
      {hasResults && mappedResults?.length > 0 ? (
        <>
          {resultsTitle && (
            <span className="block mb-4 text-primary font-bold text-lg leading-5 max-w-max">{resultsTitle}</span>
          )}

          <div
            className={`${itemsContainerClasses({ displayVariant: resultsDisplayVariant })}`}
        ...
        <p
          ...
          leading-5 w-full`}
        >
          {noResultsFoundLabel} <span className="font-bold">&ldquo;{query}&rdquo;</span>
        </p>
      )}
      ...
      {hasSuggestions && mappedSuggestions?.length > 0 && (
        <div className="mt-6">
          {suggestionsTitle && (
            <span className="block mb-4 text-primary font-bold text-lg leading-5 max-w-max">{suggestionsTitle}</span>
          )}

          <div
            className={`${itemsContainerClasses({ displayVariant: suggestionsDisplayVariant })}`}
