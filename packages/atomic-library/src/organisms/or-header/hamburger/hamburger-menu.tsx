...
export const HamburgerMenuWrapper = ({
  ...
  return (
    <div className={containerClasses({ isMobileMenuOpen })}>
      <div className="flex flex-col gap-6">
        ...
        />
        <div className="py-3">
          <div className="flex flex-col justify-start gap-6 md:gap-0 md:flex-row md:justify-end md:items-center">
            {topLinks?.map((link, idx) => {
              const isLast = idx === topLinks.length - 1
              return (
                <React.Fragment key={`${link.label}-${idx}`}>
                  <AtLink {...link} onClick={handleCloseMenu} className="text-white hover:text-white" />
                  {!isLast && <AtDivider className="border-l !border-secondary h-6 mx-3 hidden md:block" />}
                </React.Fragment>
              )
            })}
