...
describe("organisms/or-image-interactive-block", () => {
  it("should render correctly with images and subtitle", () => {
    const { getByText, queryAllByAltText } = render(
      <OrImageInteractiveBlock {...defaultProps} />
    );

    expect(getByText(defaultProps.subtitle)).toBeInTheDocument();
    expect(getByText(bodyText)).toBeInTheDocument();
    expect(
      queryAllByAltText(defaultProps.images[0].image.alt as string)
    ).toHaveLength(2); // Mobile + Desktop
    expect(
      queryAllByAltText(defaultProps.images[1].image.alt as string)
    ).toHaveLength(2); // Mobile + Desktop
  });
});
