...
describe('molecules/ml-video-player', () => {
  ...
  it('displays thumbnail before video is played', () => {
    const { getByTestId } = render(<MlVideoPlayer videoUrl={validVideoUrl} thumbnail={mockThumbnail} />)
  ...
  it('loads video iframe upon play button click', async () => {
    ...
    expect(iframe).toHaveAttribute('src', 'https://player.vimeo.com/video/946753134?autoplay=1')
  })

  it('hides play button and thumbnail after video is loaded', () => {
    const { getByRole, queryByRole } = render(<MlVideoPlayer videoUrl={validVideoUrl} thumbnail={mockThumbnail} />)
    const playButton = getByRole('button')
    fireEvent.click(playButton)
    const iframe = getByRole('iframe')
