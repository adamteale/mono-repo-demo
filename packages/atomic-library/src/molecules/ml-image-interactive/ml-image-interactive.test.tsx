...
describe('molecules/ml-image-interactive', () => {
  describe('when it is loaded', () => {
    it('should render correctly', async () => {
      ...
      expect(await render.findAllByAltText(defaultProps.image.alt)).toBeTruthy()
  ...
  describe('when user interacts with image', () => {
    it('should zoom on click', async () => {
      ...
      await user.click(render.getByRole('button', { name: defaultProps.ariaLabel }))
      expect(render.container.querySelector('img')?.style.getPropertyValue('scale')).toBe('2')

      await user.click(render.getByRole('button', { name: defaultProps.ariaLabel }))
      expect(render.container.querySelector('img')?.style.getPropertyValue('scale')).toBeFalsy()
    })
