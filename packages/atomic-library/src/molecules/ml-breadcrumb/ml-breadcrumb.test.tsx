  ...
  describe("when it receives more than 1 link", () => {
    it("should render (links.length  - 1) links in desktop and 1 in mobile", () => {
      ...
      container.querySelectorAll("a").forEach((node) => {
        if (node.dataset.testid?.includes("desktop-breadcrumb-link"))
          desktopLinkNodes.push(node);
        else if (node.dataset.testid?.includes("mobile-breadcrumb-link"))
          mobileLinkNodes.push(node);
      });
