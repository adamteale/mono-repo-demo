...
describe('molecules/ml-input-quantity', () => {
  describe('when the component is created with default props', () => {
    it('should render correctly', () => {
      const { getByTestId, getByDisplayValue } = render(<MlInputQuantity minValue={0} initialValue={0} />)
    ...
    it('should change quantity when clicking buttons', () => {
      const { getAllByRole, queryByDisplayValue } = render(<MlInputQuantity minValue={0} initialValue={0} />)
