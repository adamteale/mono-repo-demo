...
import { CreateOrderDto, CustomerDataDto, SetShippingMethodDto } from '../../../types'

export const addToBasket = async (productId: string, quantity: number, variantId?: string) => {
...
export const updateBasketItem = async (basketItemId: string, quantity: number) => {
  const basketId = getStorageState(LOCAL_BASKET_ID)
  const accessToken = await resolveAccessToken()
...
export const setShippingMethod = async (shippingMethodDto: SetShippingMethodDto) => {
  const basketId = getStorageState(LOCAL_BASKET_ID)
  const accessToken = await resolveAccessToken()
