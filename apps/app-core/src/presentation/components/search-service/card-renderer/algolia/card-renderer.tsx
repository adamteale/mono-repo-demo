...
import { SearchResponse } from "@search/algolia/provider/types";
...
export const MlProductCardRenderer = ({ hit }: MlProductCardRendererProps) => {
  ...
  const addToBasket = useMemo(() => {
    ...
    return () =>
      addProductToBasket(hit.productId, 1, variant.sku, variant.variantId);
  }, [
    colorsValues.length,
    sizes,
    hit.variants,
  ...
  const brandLink = brand
    ? { label: brand, href: `products/${hit.slug}` }
    : undefined;
