...
export const updateBasketItem = async (
  ...
  const basketId = getStorageState(LOCAL_BASKET_ID);
  const accessToken = await resolveAccessToken();
  const updatedBasket = await updateItemQuantity(
    basketItemId,
    quantity,
    basketId,
    accessToken
...
export const removeItemFromBasket = async (basketItemId: string) => {
  const basketId = getStorageState(LOCAL_BASKET_ID);
  const accessToken = await resolveAccessToken();
  const updatedBasket = await deleteItemFromBasket(
    basketId,
    basketItemId,
    accessToken
  );
...
export const updateCustomerData = async (customerDataDto: CustomerDataDto) => {
  const basketId = getStorageState(LOCAL_BASKET_ID);
  const accessToken = await resolveAccessToken();
  const response = await updateBasketCustomerData(
    basketId,
    accessToken,
    customerDataDto
  );
...
export const setShippingMethod = async (
  ...
  const basketId = getStorageState(LOCAL_BASKET_ID);
  const accessToken = await resolveAccessToken();
  const response = await setBasketShippingMethod(
    basketId,
    accessToken,
    shippingMethodDto
  );
...
export const createOrder = async (createOrderDto: CreateOrderDto) => {
  const basketId = getStorageState(LOCAL_BASKET_ID);
  const accessToken = await resolveAccessToken();
  const response = await createBasketOrder(
    basketId,
    accessToken,
    createOrderDto
  );
